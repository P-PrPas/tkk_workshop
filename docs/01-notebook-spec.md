...
```
> **`lapx` ต้องลงตั้งแต่เซลล์แรก** — ByteTrack (พาร์ท 4) ต้องใช้ ถ้าไม่ลงไว้ ultralytics
> จะ `pip install` ให้เองตอนเรียก `model.track()` ครั้งแรก = ค้างกลางเดโมนาทีที่ 70
...
```
> **GPU เป็น opt-in ไม่ใช่ข้อบังคับ และไม่ค่อยคุ้ม** — เทรนจบ ~15 วิ บน CPU อยู่แล้ว
> โค้ดไม่ hardcode `device` เลย ultralytics ใช้ GPU อัตโนมัติถ้ามี
> เซลล์กล้อง realtime **T4 ช่วยแค่นิดเดียว** (~4→~7 fps) เพราะคอขวดคือ `eval_js` ไม่ใช่ inference
> เหตุที่ไม่บังคับสลับ T4: (1) T4 ฟรีอาจถูกปฏิเสธ/โดน quota กลางคาบ ถ้าโน้ตบุ๊ก *ต้อง* มี GPU
> คนกลุ่มนั้นตกขบวน (2) 20 คนกด Change runtime type พร้อมกัน = งง + เสียเวลา
> สรุป: ปล่อยให้เป็น CPU เป็น default ใครอยากลอง T4 ก็ได้ แต่อย่าคาดหวังว่าจะ real-time
...
### เซลล์ 9 (markdown) — Precision / Recall คืออะไร
ตาราง 2 แถว: Precision = *ที่ทายไป ถูกกี่ %* · Recall = *ของจริงที่มี เจอกี่ %*
พร้อมนิยาม TP/FP/FN และประโยคที่ต้องพูด: **สองตัวนี้ขัดกันเสมอ** ลด `conf` → Recall ขึ้น
Precision ตก · งานตรวจชิ้นงานยอมทายเกินดีกว่ามองข้าม (เอา Recall)

### เซลล์ 10 (code) — Precision / Recall นับให้เห็นที่มา
**นับ TP/FP/FN เองบนรูป test** (จับคู่ pred กับ label ด้วย IoU ≥ 0.5 แบบ greedy)
แล้วพิมพ์เศษส่วนออกมาเต็ม ๆ `Precision = 3/(3+1) = 0.75` + กราฟแท่งสองแท่ง
ปิดท้ายด้วยตัวเลขทางการจาก `model.val(split="test", classes=[41])` (`box.mp` / `box.mr`
/ `box.map50`) ไว้เทียบ
> เหตุที่นับเอง ไม่พิมพ์ค่าจาก `val()` อย่างเดียว: ผู้เรียนต้องเห็นว่าตัวเลขพวกนี้
> **มาจากการนับกล่องทีละใบ** ไม่ใช่เวทมนตร์ของไลบรารี · ตัวเลขจากรูป 3 ใบเชื่อไม่ได้
> แต่ *วิธีนับ* ใช้ได้ทุกขนาดข้อมูล — markdown ถัดไปพูดตรงนี้
...
> **timestamp ของ `detect_for_video` ต้องเพิ่มขึ้นเรื่อย ๆ ข้ามทุกเซลล์** (พาร์ท 2 กับ 3 ใช้
> `landmarker` ตัวเดียวกัน) — เซลล์นี้นิยาม `video_ts()` ที่อิง `time.monotonic()` ให้ทั้งสอง
> พาร์ทเรียก · ถ้าใช้ตัวนับแยก (`_t`, `_tt`) พาร์ท 3 จะส่ง timestamp ย้อนหลัง →
> `detect_for_video` โยน error → เซลล์กล้องพาร์ท 3 ดับทันที
...
def hand_on_cup(lm, w, h, cup_boxes, min_pts=8, margin=0.3):
    """จุดบนมือตกในกรอบแก้ว (เผื่อขอบ margin) ตั้งแต่ min_pts จุด = กำลังถือ"""
```
> เดิมกฎคือ "มือ**กำ** + กรอบทับกัน" แต่ทดสอบแล้วพัง: แก้วไม่มีหูต้องจับตรง ๆ
> `count_extended` อ่านได้เป็น OPEN → ไม่เคยขึ้น HOLDING เลย
> เปลี่ยนมานับจุด landmark ที่ตกในกรอบแทน `hand_state` เหลือไว้แค่ระบายสีโครงมือ
> (เป็น teaching moment: กฎแรกที่คิดออกมักไม่รอดของจริง)
...
## พาร์ท 4 — เช็กลิสต์ผู้ตรวจ ("ชิ้นไหนถูกหยิบไปตรวจแล้ว")

### เซลล์ 18 (markdown) — เปลี่ยนคำถาม
`HOLDING` ตอบได้แค่ "ตอนนี้" แต่คำถามของหัวหน้างานคือ *"ครบหรือยัง ชิ้นไหนยังไม่ได้แตะ"*
คำถามนี้ต้องรู้ว่าแก้วใบนี้กับใบเมื่อกี้เป็นใบเดียวกัน = **tracking**
เปลี่ยนคำเดียว `model(...)` → `model.track(..., persist=True)` แล้วได้ `boxes.id` มาเลย

### เซลล์ 19 (code) — realtime เช็กลิสต์
```python
trails = defaultdict(lambda: deque(maxlen=40))   # เส้นทางรายใบ
picked = set()                                   # ใบที่ถูกหยิบแล้ว

def new_round():                                 # จุด reset — รันเซลล์ซ้ำ = รอบใหม่
    trails.clear(); picked.clear()
```
วาด: polyline เส้นทาง · กรอบ **เหลือง = ยังไม่ตรวจ / เขียว = ตรวจแล้ว** · `cup <id>` +
**เครื่องหมายถูกวาดเองด้วย `cv2.line` สองเส้น** (ฟอนต์ Hershey ของ OpenCV ไม่มี glyph `✓`)
· ป้ายนับ `checked n/m` มุมบนซ้าย
> ติ๊กทันทีที่ `hand_on_cup` เป็นจริงหนึ่งเฟรม — **จงใจให้หลวม** เพื่อให้ผู้เรียนเห็นเองว่า
> เอามือกวาดผ่านก็ติ๊ก แล้วแอปค่อยเฉลยว่าแก้ด้วยตัวนับสะสม (`pick_frames`)

### เซลล์ 20 (markdown) — ส่งไม้ต่อให้แอป
ให้ผู้เรียนสังเกตด้วยตาตัวเองว่ามีปัญหาอะไรบ้าง แล้วค่อยเฉลย:
1. ป้ายกระพริบตลอด (ไม่มีความจำข้ามเฟรม)
2. มือเฉียดผ่านหน้าแก้วก็ติ๊กถูกแล้ว ทั้งที่ยังไม่ได้หยิบขึ้นมาดู
3. มือบังแก้วจนตรวจไม่เจอ → กลับมาได้ id ใหม่ กลายเป็นชิ้นที่ยังไม่ตรวจ
4. ช้า
5. อยากเริ่มรอบใหม่ต้องกดรันเซลล์ใหม่ (ไม่มีปุ่ม)
6. ถอดปลั๊กกล้องแล้วทุกอย่างพัง

> "หกข้อนี้แหละครับ คือระยะห่างระหว่าง demo กับ product เดี๋ยวผมโชว์ตัวที่แก้ครบแล้ว"
